...
- Color theme only → `localStorage` (`src/constants/storage.js` +
  `src/hooks/useTheme.js`), applied as `<html data-theme="...">`.

**Adding a color theme** requires two edits in sync: an entry in
`THEMES` (`src/constants/themes.js`) for the picker UI, and a matching
`[data-theme="<id>"]` variable block in `src/styles/themes.css`.
